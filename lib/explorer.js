...
module.exports = {
  ...
  get_block_with_transactions: function (hash, cb) {
    var cmd = prepareRpcCommand(settings.api_cmds.getblock, (hash ? [hash, 2] : []));
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        // Fallback or specific URI if needed, assuming RPC mostly
        var uri = base_url + 'getblock?hash=' + hash;
