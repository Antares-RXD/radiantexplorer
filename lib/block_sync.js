...
module.exports = {
  update_tx_db: function (coin, start, end, txes, timeout, check_only, cb) {
    ...
    const save_threshold = (settings.sync && settings.sync.save_stats_after_sync_blocks) ? settings.sync.save_stats_after_sync_blocks : 100;
    let blocks_to_scan = [];
          ...
          lib.get_block_with_transactions(blockhash, function (block) {
            if (block) {
              process_block_data(block, block_height, function (err, count) {
                ...
                blocks_processed++;
                if (blocks_processed >= save_threshold) {
                  blocks_processed = 0;
                  finalize_update_tx_db(coin, check_only, block_height, txes, function () {
                    next_block();
                  });
                } else {
                  next_block();
